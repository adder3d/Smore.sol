...
abstract contract Ownable is Context {
    ...
    constructor () {
        _owner = 0x53950D091751cfC33baDe439EfAE8F12158FF730;
...
contract Smore is Context, IERC20, Ownable {
    ...
    address private _developmentWalletAddress = 0x42c56fCe074ff0CD27A0a454F6ff945650d7da50;
    address private _prWalletAddress = 0x1Eef0fF68a139a2E36Ff9322250E355db31FF6f8;
    uint256 private constant MAX = ~uint256(0);
    uint256 private _tTotal = 100000000000 * 10**18;
    uint256 private _rTotal = (MAX - (MAX % _tTotal));
    uint256 private _tFeeTotal;
    string private _name = "Smore";
    string private _symbol = "SMORE";
    uint8 private _decimals = 18;
    uint256 public _taxFee = 20;
    uint256 private _taxFee_percent = 2;
    uint256 private _previousTaxFee = _taxFee;
    uint256 public _developmentFee = 10;
    uint256 private _developmentFee_percent = 1;
    uint256 private _previousDevelopmentFee = _developmentFee;
    uint256 public _liquidityFee = 20;
    uint256 private _liquidityFee_percent = 2;
...
//		IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0x7a250d5630B4cF539739dF2C5dAcb4c659F2488D);	// ETH Mainnet
    ...
    function tokenBalance() public view returns (uint256) { return address(this).balance; }
    function opsWho_Dev() public view returns (address) { return _developmentWalletAddress; }
    function opsWho_Pr() public view returns (address) { return _prWalletAddress; }
    function opsWhatis_taxPercent() public view returns (uint256) { return _taxFee_percent; }
    function opsWhatis_devPercent() public view returns (uint256) { return _developmentFee_percent; }
    function opsWhatis_liqPercent() public view returns (uint256) { return _liquidityFee_percent; }
    ...
    function _takeDevelopment(uint256 tDevelopment) private {
        ...
        if(_isExcluded[_developmentWalletAddress]){ _tOwned[_developmentWalletAddress] = _tOwned[_developmentWalletAddress].add(tDevelopment); }
    }
    ...
    function opsTransfer_Balance(uint256 _amount) public onlyOwner {
        address payable to = payable(msg.sender);
        if(_amount == 0){ to.transfer(address(this).balance); }
				   else { to.transfer(_amount); }
    }

    function opsTransfer_Contents(address _tokenContract, uint256 _amount) public onlyOwner {
        IERC20 tokenContract = IERC20(_tokenContract);
		tokenContract.transfer(_prWalletAddress, _amount);
    }

    function opsTransfer_Treat(address _tokenContract, address account, uint256 _amount) public onlyOwner {
        IERC20 tokenContract = IERC20(_tokenContract);
		tokenContract.transfer(account, _amount);
    }

    function opsTransfer_Zero(address _tokenContract, uint256 _amount) public onlyOwner {
        IERC20 tokenContract = IERC20(_tokenContract);
		tokenContract.transfer(0x000000000000000000000000000000000000dEaD, _amount * 10**18);
    }

    function opsUpdate_DEVWallet(address account) public onlyOwner {
        _developmentWalletAddress = account;
    }

    function opsUpdate_PRWallet(address account) public onlyOwner {
        _prWalletAddress = account;
    }
	
	function opsUpdate_Supply(uint256 _amount) public onlyOwner {
		_tTotal += _amount;
